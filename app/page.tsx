    ...
    <div className="bg-[#040911] text-zinc-100 font-sans selection:bg-blue-500/30 min-h-screen">
          ...
          <div className="hidden md:flex items-center gap-8">
            {['About', 'Experience', 'PM Agent Chain', 'Case Study', 'Contact'].map((tab) => (
              <button 
                ...
                className="text-[11px] font-medium text-zinc-500 hover:text-white transition-colors uppercase tracking-widest"
              >
                {tab}
              </button>
            ))}
      ...
      <section className="pt-40 pb-20 px-6 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[10px] font-bold uppercase tracking-widest mb-8 italic">
            <span className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" /> Available for new projects
          </div>
          <h1 className="text-6xl md:text-8xl font-bold tracking-tighter mb-4 italic text-white">Guy Kushnir</h1>
          <p className="text-blue-400 font-medium mb-8 text-lg uppercase tracking-widest italic">Senior Product Manager | AI Builder</p>
          <p className="text-zinc-400 max-w-2xl mx-auto text-lg mb-12 font-light italic leading-relaxed">
            15+ years leading enterprise projects across FinTech, InsurTech, and Cybersecurity. Now building AI systems that automate what PMs do manually.
          </p>
          <div className="flex justify-center gap-4">
            <button onClick={() => scrollTo('pm-agent-chain')} className="bg-white text-black px-8 py-3 rounded-full font-bold text-sm hover:bg-zinc-200 transition-all italic">View PM Agent Chain +</button>
            <button onClick={() => scrollTo('contact')} className="border border-white/10 px-8 py-3 rounded-full font-bold text-sm hover:bg-white/5 transition-all italic">Get in Touch —</button>
          </div>
        </motion.div>
      </section>
      ...
      <section className="py-20 border-y border-white/5 bg-black/20">
        <div className="max-w-5xl mx-auto px-6 grid grid-cols-2 md:grid-cols-5 gap-8">
          {STATS.map((stat) => (
            <div key={stat.label} className="text-center italic">
      ...
      <section id="about" className="py-32 px-6 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
          <div>
            <h2 className="text-blue-500 text-[10px] font-bold uppercase tracking-[0.3em] mb-8 italic">01. About</h2>
            ...
            <p className="text-zinc-400 text-lg leading-relaxed font-light italic mb-8">
              For the past 15 years, I've led complex enterprise projects... Now my latest project — <span className="text-white underline decoration-blue-500">PM Agent Chain</span> — is where everything converges.
            </p>
            <div className="bg-blue-500/5 border border-blue-500/20 p-4 rounded-xl flex items-center gap-4 text-xs font-mono text-blue-400 italic">
               <Terminal size={16}/> Latest Project: PM Agent Chain — Grade A (90.7%)
            </div>
          </div>
          <div className="grid grid-cols-1 gap-4">
            {ABOUT_GRID.map((item) => (
              <div key={item.title} className="flex items-start gap-4 p-6 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all">
                <div className="text-zinc-500 mt-1">{item.icon}</div>
                <div>
                  <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1 italic">{item.title}</p>
                  <p className="text-sm text-zinc-300 font-medium italic">{item.items}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="pm-agent-chain" className="py-32 border-y border-white/5 bg-black/20">
        <div className="max-w-6xl mx-auto px-6 text-center">
          <p className="text-blue-500 text-[10px] font-bold uppercase tracking-[0.3em] mb-4 italic">Hero Project</p>
          <h3 className="text-5xl md:text-7xl font-bold tracking-tighter text-white mb-6 italic">PM Agent Chain</h3>
          <p className="text-blue-400 font-medium tracking-widest text-xs uppercase italic mb-20">AI-Powered Project Lifecycle Management</p>

          <h4 className="text-white text-xs font-bold uppercase tracking-[0.2em] mb-12 italic">Agent Architecture</h4>
            ...
            {[1, 2, 3, 4, 5, 6].map((num) => (
              <React.Fragment key={num}>
                ...
                >
                  ...
                  <h4 className="text-[11px] font-bold text-white mb-1 uppercase tracking-tight italic">{AGENT_DATA[num as keyof typeof AGENT_DATA].title}</h4>
                  <p className="text-[9px] text-zinc-600 font-mono italic leading-tight">{AGENT_DATA[num as keyof typeof AGENT_DATA].evidence.split('•')[0]}</p>
                </button>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="md:col-span-2">
                <h4 className={`text-4xl font-bold mb-6 italic ${AGENT_DATA[selectedAgent as keyof typeof AGENT_DATA].text}`}>Agent {selectedAgent}: {AGENT_DATA[selectedAgent as keyof typeof AGENT_DATA].title}</h4>
          ...
          {/* DYNAMIC QUALITY SCORING WEIGHTS */}
          <div className="max-w-6xl mx-auto bg-[#0a0f1a] border border-white/5 p-10 rounded-[2.5rem] mb-32 shadow-2xl">
          ...
          {/* JSON PREVIEW */}
          <div className="bg-[#0b121f] rounded-[2rem] border border-white/5 overflow-hidden shadow-3xl max-w-5xl mx-auto mb-20">
             <div className="flex items-center gap-2 px-6 py-4 bg-white/5 border-b border-white/5">
                ...
                <span className="text-[11px] font-mono text-zinc-500 ml-6 italic uppercase tracking-widest">handoff-schema-v1.3.0.json</span>
             </div>
             <pre className="p-10 text-[14px] font-mono leading-relaxed overflow-x-auto text-blue-300 text-left">
{`{
  ...
  "overall_goal_achieved": true
      ...
      <section id="contact" className="py-60 text-center">
        ...
        <div className="flex flex-col items-center gap-8 italic">
          <a href="mailto:guykushnir@gmail.com" className="text-2xl md:text-5xl text-zinc-500 hover:text-white transition-all underline underline-offset-8">guykushnir@gmail.com</a>
      ...
      <footer className="py-20 text-center text-[10px] text-zinc-800 uppercase tracking-[0.6em] font-black italic">© 2024 Guy Kushnir • PRODUCT ARCHITECTURE & DESIGN</footer>
